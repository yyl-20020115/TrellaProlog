...
bool pl_isatty(prolog* pl) { return _isatty(_fileno(pl->streams[0].fp)); }
...
bool pl_query(prolog *pl, const char *s, struct pl_sub_query **subq)
{
	if (!pl || !*s || !subq)
		return false;
	...
	pl->is_query = true;
	bool ok = run(pl->p, s, true);
	*subq = (struct pl_sub_query*)pl->curr_query;
	if (get_status(pl)) pl->curr_m = pl->p->m;
	return ok;
}

bool pl_redo(struct pl_sub_query *subq)
{
	if (!subq)
		return false;
...
bool pl_done(struct pl_sub_query *subq)
{
	if (!subq)
		return false;
...
prolog *pl_create()
{
	...
	CHECK_SENTINEL(pl->streams[0].filename = _strdup("stdin"), NULL);
	CHECK_SENTINEL(pl->streams[0].mode = _strdup("read"), NULL);
	map_set(pl->streams[0].alias, _strdup("user_input"), NULL);
	...
	CHECK_SENTINEL(pl->streams[1].filename = _strdup("stdout"), NULL);
	CHECK_SENTINEL(pl->streams[1].mode = _strdup("append"), NULL);
	map_set(pl->streams[1].alias, _strdup("user_output"), NULL);
	...
	CHECK_SENTINEL(pl->streams[2].filename = _strdup("stderr"), NULL);
	CHECK_SENTINEL(pl->streams[2].mode = _strdup("append"), NULL);
	map_set(pl->streams[2].alias, _strdup("user_error"), NULL);
