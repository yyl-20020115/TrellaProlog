...
static bool fn_iso_atom_chars_2(query *q)
{
	...
	if (!is_var(p2) && is_var(p1)) {
		...
		cell tmp;
		check_heap_error2(make_cstring(&tmp, SB_cstr(pr)), SB_free(pr));
		SB_free(pr);
		bool ok = unify(q, p1, p1_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		return ok;
...
static bool fn_iso_atom_codes_2(query *q)
{
	...
	if (!is_var(p2) && is_var(p1)) {
		...
		cell tmp;
		check_heap_error2(make_cstringn(&tmp, SB_cstr(pr), SB_strlen(pr)), SB_free(pr));
		SB_free(pr);
		bool ok = unify(q, p1, p1_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		return ok;
...
static bool fn_hex_bytes_2(query *q)
{
	...
	if (!is_var(p2) && is_var(p1)) {
		...
		cell tmp;
		check_heap_error2(make_string(&tmp, SB_cstr(pr)), SB_free(pr));
		SB_free(pr);
		bool ok = unify(q, p1, p1_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		return ok;
...
static void save_name(FILE *fp, query *q, pl_idx_t name, unsigned arity)
{
	...
	for (predicate *pr = m->head; pr; pr = pr->next) {
		if (pr->is_prebuilt && (arity == ~0U))
			continue;

		if (name != pr->key.val_off)
			continue;

		if ((arity != pr->key.arity) && (arity != ~0U))
			continue;
...
static int do_b64encode_2(query *q)
{
	...
	cell tmp;
	check_heap_error2(make_string(&tmp, dstbuf), free(dstbuf));
	free(dstbuf);
	bool ok = unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	unshare_cell(&tmp);
	return ok;
...
static bool do_urlencode_2(query *q)
{
	...
	if (is_string(p1))
		...
		check_heap_error2(make_cstring(&tmp, dstbuf), free(dstbuf));
...
static bool fn_atom_lower_2(query *q)
{
	...
	*dst = '\0';
	cell tmp;
	check_heap_error2(make_cstringn(&tmp, tmps, C_STRLEN(q, p1)), free(tmps));
	free(tmps);
	bool ok = unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	unshare_cell(&tmp);
	return ok;
...
static bool fn_string_lower_2(query *q)
{
	...
	*dst = '\0';
	cell tmp;
	check_heap_error2(make_stringn(&tmp, tmps, C_STRLEN(q, p1)), free(tmps));
	free(tmps);
	bool ok = unify(q, p2, p2_ctx, &tmp, q->st.curr_frame);
	unshare_cell(&tmp);
	return ok;
...
static bool fn_replace_4(query *q)
{
	...
	if (SB_strlen(pr))
		check_heap_error2(make_stringn(&tmp, SB_cstr(pr), SB_strlen(pr)), SB_free(pr));
	else
		make_atom(&tmp, g_nil_s);
...
static bool fn_get_unbuffered_code_1(query *q)
{
	...
	int ch = history_getch_fd(_fileno(str->fp));
