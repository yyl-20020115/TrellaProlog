...
typedef struct { void* p; } pl_sub_query;
...
extern bool pl_query(prolog*, const char *expr,struct pl_sub_query **q);
extern bool pl_redo(struct pl_sub_query *q);
extern bool pl_done(struct pl_sub_query *q);	// only call if redo still active
